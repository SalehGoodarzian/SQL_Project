...
SELECT * FROM ALBUMS WHERE RELEASE_YEAR IS NOT NULL ORDER BY RELEASE_YEAR;
...
SELECT * FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID;
SELECT BANDS.NAME FROM BANDS LEFT JOIN ALBUMS ON BANDS.ID=ALBUMS.BAND_ID WHERE ALBUMS.ID IS NULL;

-- QUESTION 6: Get the Longest Album
SELECT * FROM SONGS LEFT JOIN ALBUMS ON SONGS.ALBUM_ID=ALBUMS.ID;
SELECT ALBUMS.NAME, SUM(SONGS.LENGTH) 
FROM SONGS LEFT JOIN ALBUMS 
ON SONGS.ALBUM_ID=ALBUMS.ID 
GROUP BY ALBUMS.NAME 
ORDER BY SUM(SONGS.LENGTH) DESC LIMIT 1;

-- QUESTION 7: Update the Release Year of the Album with no Release Year to 2000
SELECT * FROM ALBUMS;
...
INSERT INTO BANDS(ID,NAME) VALUES (8, 'HABIB');
INSERT INTO ALBUMS(ID,NAME, RELEASE_YEAR,BAND_ID) VALUES (19, 'KAVIR', 1999, 8);
...
SELECT AVG(LENGTH) AS 'Average length of all songs' FROM SONGS;

-- QUESTION 11: Select the longest Song of each Album
SELECT * FROM SONGS LEFT JOIN ALBUMS ON SONGS.ALBUM_ID=ALBUMS.ID;
SELECT ALBUMS.NAME AS 'Album\'s name', SONGS.NAME AS 'Song\'s name', MAX(SONGS.LENGTH) AS 'Length', ALBUMS.RELEASE_YEAR AS 'Release year'
FROM SONGS LEFT JOIN ALBUMS 
ON SONGS.ALBUM_ID=ALBUMS.ID
GROUP BY ALBUMS.NAME 
ORDER BY MAX(SONGS.LENGTH) DESC;

-- Before I do Question 12, I'd like to do a practice...Get the number of albums for each Band
SELECT * FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID;
SELECT BANDS.NAME AS 'Band\'s name', COUNT(ALBUMS.NAME) AS 'Number of albums'
FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID
GROUP BY BANDS.NAME;

-- QUESTION 12: Get the number of Songs for each Band
SELECT * FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID LEFT JOIN SONGS ON SONGS.ALBUM_ID=ALBUMS.ID;
SELECT BANDS.NAME AS 'Band\'s name', COUNT(SONGS.NAME) AS 'Number of songs'
FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID LEFT JOIN SONGS ON SONGS.ALBUM_ID=ALBUMS.ID
GROUP BY BANDS.NAME
ORDER BY COUNT(SONGS.NAME) DESC;

-- QUESTION 13: Get the total length of music each band produced
SELECT * FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID LEFT JOIN SONGS ON SONGS.ALBUM_ID=ALBUMS.ID;
SELECT BANDS.NAME AS 'Band\'s name', SUM(SONGS.LENGTH) AS 'Length of music'
FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID LEFT JOIN SONGS ON SONGS.ALBUM_ID=ALBUMS.ID
GROUP BY BANDS.NAME
ORDER BY SUM(SONGS.LENGTH) DESC;

-- QUESTION 13: Get the longest song of each band
SELECT * FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID LEFT JOIN SONGS ON SONGS.ALBUM_ID=ALBUMS.ID;
SELECT BANDS.NAME, MAX(SONGS.LENGTH) FROM BANDS LEFT JOIN ALBUMS ON ALBUMS.BAND_ID=BANDS.ID LEFT JOIN SONGS ON SONGS.ALBUM_ID=ALBUMS.ID
GROUP BY BANDS.NAME
ORDER BY MAX(SONGS.LENGTH) DESC;